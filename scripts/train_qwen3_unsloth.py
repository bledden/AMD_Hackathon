#!/usr/bin/env python3
"""
Fine-tune Qwen3-235B with Unsloth + LoRA + Replay Buffer + Curriculum Learning
...
Uses Unsloth for ROCm compatibility and 2x faster training
"""
...
def load_model_and_tokenizer(model_name: str = "Qwen/Qwen3-235B-A22B-Instruct-2507"):
    """
    Load Qwen3-235B with Unsloth for ROCm-compatible 4-bit quantization
    ...
    # Reserve ~10GB VRAM for LoRA adapters and training
    # With rank=32 LoRA, we need less headroom
    max_memory = {0: "180GB", "cpu": "100GB"}  # Leave 12GB VRAM free

    logging.info(f"   Memory strategy: Max 180GB VRAM, offload excess to CPU")
    ...
    logging.info(f"   Free for LoRA: {192 - vram_used:.2f}GB")
...
def curriculum_training(
    ...
    for chunk_idx in range(n_chunks):
        ...
        # Training arguments (reduced batch size to save VRAM)
        training_args = TrainingArguments(
            output_dir=str(output_path / f"checkpoint_chunk{chunk_idx}"),
            num_train_epochs=1,
            per_device_train_batch_size=1,  # Reduced from 2 to save VRAM
            gradient_accumulation_steps=16,  # Doubled to maintain effective batch size
            learning_rate=lr,
            warmup_steps=50,
            logging_steps=10,
            save_steps=500,
            save_total_limit=2,
            fp16=False,
            bf16=True,  # Model is in bfloat16 precision
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("🎯 QWEN3-235B FINE-TUNING: Unsloth + LoRA + Replay + Curriculum")
    logging.info("=" * 80)

    # Paths
    train_path = Path('data/curriculum/train_45k.json')
    output_dir = 'models/qwen3_235b_unsloth_curriculum'
    ...
    # Setup LoRA with Unsloth (reduced rank to save VRAM)
    model = setup_lora(model, rank=32, alpha=64)
